    ...
    ?>
      ...
      <tr align="Center"   <? if($rs[$i]['service_status']==5 or $rs[$i]['service_status']==6 )  {  ?> style="background:#CCCCCC;" <? }
        ...
        <td>
          <strong>
                <? 
                if($rs[0]['fwd_tech_rm_id'] != "" && $rs[0]['fwd_repair_id'] == ""){
                    $support_query = select_query("select user_name from request_forward_list where user_id='".$rs[0]['fwd_tech_rm_id']."'");
                    $forward_name = $support_query[0]['user_name'];
                } else if($rs[0]['fwd_repair_id'] != "") {
                    $support_query = select_query("select user_name from request_forward_list where user_id='".$rs[0]['fwd_repair_id']."'");
                    $forward_name = $support_query[0]['user_name'];
                } else { $forward_name = '';}
