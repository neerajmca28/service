...
if($mode=='') { $mode="new"; }
  ...
  if($mode=='close')
	...
	$rs = select_query("SELECT * FROM installation where (installation_status='2'  or installation_status='21') and (branch_id=".$_SESSION['BranchId'] ." or inter_branch=".$_SESSION['BranchId'].") order by id desc");
 ...
 <?}?>
	...
	elseif($rs[$i]['required']=='urgent' && $rs[$i]['fwd_repair_to_install']==''){ ?>style="background:#ADFF2F" <? } elseif($rs[$i]['installation_status']=='21' && $rs[$i]['fwd_repair_to_install']!=''){ echo 'style="background-color:#FFC184"';}?> >
	<td><?php echo $i+1; ?></td>
	 ...
	 <td align="center" nowrap>
		...
		?>
         ...
         <td><a href="#" onclick="Show_record(<?php echo $rs[$i]["id"];?>,'installation','popup1'); " class="topopup">View Detail</a>
          <?php if($rs[$i]['fwd_install_to_repair']=="" && $rs[$i]['fwd_repair_to_install']=="" && $rs[$i]['installation_status']==21){?> 
    ...
    <?php } ?>
	   ...
	   {?>
        ...
        {?>
        <?php
         if($rs[$i]["installation_status"]==21 && $rs[$i]["inter_branch"]==0 || $rs[$i]["inter_branch"]==$_SESSION['BranchId'])
         {?>
		...
		<?php }
		elseif($rs[$i]["installation_status"]==2 && $rs[$i]["inter_branch"]==0 || $rs[$i]["inter_branch"]==$_SESSION['BranchId'])
		{?>
        <td >Edit  </td>
