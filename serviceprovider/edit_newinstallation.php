...
if(isset($_POST['submit']))
{
    //echo '<pre>';print_r($_POST); die;
        ...
        $rtime=date('Y-m-d H:i:s',strtotime($_POST['rtime']));
        ...
        $branchid=addslashes($_POST['sim_no']);
...
if($errMSG=="")
    {
       ...
       mysql_query("update installer set status=0 where inst_id='".$inst_id1."'",$dblink2);
        ...
        // done_required='$done_required',data_check_string='$data_check' WHERE id='$id'",$dblink2);
               ...
               mysql_query("UPDATE installation SET rtime='$rtime',close_date='".date("Y-m-d")."',inst_date='".date("Y-m-d")."',newpending='0', newstatus='0', device_type='$device_type',installation_made='$total_inst_made', installation_status=5, sim_change_status='$sim_change',data_check_string='$data_check' WHERE id='$id'",$dblink2);
...
if(isset($_POST['update']))
{
    //echo '<pre>'; print_r($_POST); die;
        ...
        // echo "UPDATE installation SET inst_id='$inst_id', job_type='$job_type' , inst_name='$inst_name',
        // inst_cur_location='$inst_cur_location' ,billing='$billing',payment='$payment' , installation_status=2  WHERE id='$id'"; die;

        $pending=mysql_query("UPDATE installation SET inst_id='$inst_id', job_type='$job_type' , inst_name='$inst_name',inst_cur_location='$inst_cur_location', installation_status=2  WHERE id='$id'",$dblink2);
...
<form method="post" action="" name="form3">
    ...
    <table style="width: 900px;" cellspacing="2" cellpadding="3" border="1">
       ...
       <tr>
       <?php $sqlModel=select_query("SELECT device_model FROM device_model where id='".$rows[0]["model"]."' ");   ?>
          <td  align="right"><strong>Device Model:</label></strong></td>
          <td align="center"> <label><strong><?php echo $sqlModel[0]['device_model'];?></strong> </label></td>
    ...
    <!--     
        ...
        <tr>
           ...
           <table>
    <tr>
    <td><input type='text' name='Veh_name1' required></td><td>";
    <?php 
    $assign_device = select_query_inventory("select device.device_id, device.device_imei, device.itgc_id, sim.sim_no, device.dispatch_date,
                        ...
                        challandetail.CurrentRecord=1 and challandetail.installerid='".$rows[0]['inst_id']."' ");
    ?>

    <select  name='DeviceIMEI1' required>
    <option value=''>Select imei</option>
  <?php
          for($aid=0;$aid<count($assign_device);$aid++)
          {
          ?>
                <option value="<?php echo $assign_device[$aid]['device_imei']; ?>"><?php echo $assign_device[$aid]['device_imei'];?></option>
    ...
    <td>
            ...
            <td>
                  <select name='ac1' required>
                      <option value=''>AC</option>
                    <option value='on'>YES</option>
                    <option value='off'>NO</option>
                </select></td><td>
            <select name='immobilizer1' required>
                      <option value=''>Immobilizer</option>
                    <option value='Yes'>YES</option>
                    <option value='No'>NO</option>
                </select></td><td>
            <select name='immobilizer_type1' >
      ...
      <!--   
       ...
       <?php }
        ...
        <tr>
            ...
            <td><input type='text' name='Veh_name1' required></td><td>
    <?
        $deactive_query = select_query_live_con("SELECT sys_service_id FROM matrix.group_services WHERE active=0 AND sys_group_id=(SELECT sys_group_id FROM matrix.group_users where sys_user_id='".$rows[0]['user_id']."')");
   ...
   ?>
         ...
         <?
          for($ime=0;$ime<count($result);$ime++)
          {
            ...
            if($device_imei !='')
            {?>
                <option value=".$device_imei."><? echo $device_imei; ?></option>
           <? }
